...
- メイン画面からiframeモーダルとして起動し、親モーダルの右上「×」ボタンでモーダルを終了
...
- マスタ画面自体にはヘッダーエリア（タイトル行）を設けない
- タイトルは親モーダルのヘッダーで表示する
- 一覧はテーブルの `thead` と `tbody` を同一テーブル内に配置した埋め込みテーブル構造
- リスト部のスクロール時もヘッダーが `position: sticky` で固定される
...
- iframeを使用したモーダルダイアログ
- モーダルの場合：幅600px〜800px、最大高さ90vh
- 親モーダルのヘッダーにタイトルと「×」閉じるボタンを配置

### 8.3 閉じる動作

| トリガー | 動作 |
|---------|------|
| 親モーダルヘッダーの「×」ボタン | 画面を閉じる |
| Escキー | 画面を閉じる（モーダル時） |
