...
|----|---------|--------|-----------|-------------|
| 1 | 商品マスタ | master_product | master_product.php | M_商品 |
| 2 | 定型文マスタ | master_template | master_template.php | M_定型文 |
| 3 | 対応区分マスタ | master_category | master_category.php | M_対応区分 |
| 4 | 対応内容項目マスタ | master_content | master_content.php | M_対応内容項目 |

### 1.3 アクセス条件
- 管理者権限（AUTH_LEVEL >= 2）のみアクセス可能
- 一般ユーザーはメニューボタンが非表示
...
|----|-------|--------|------|------|------|
| 1 | 部門 | product-bumon | セレクト | ○ | 管理部門 |
| 2 | 商品コード | product-code | 数値 | △ | 新規時は自動採番、修正時は編集不可 |
| 3 | 商品名 | product-name | テキスト | ○ | 商品名称（255文字以内） |
| 4 | 使用区分 | product-use | セレクト | ○ | 使用する/使用しない |
